...
void treeAddNode (treeTree* t, int id, int parentid) {
	if ((t->users)[id] != NULL) {
		...
		if ((parent->children)->last != NULL) v->parent = (parent->children)->last;
		else v->parent = (parent->children)->first;
		v->parentArr = (parent->children);
	}
}
...
void treeDeleteNode (treeTree* t, int id) {
	if (id == 0) {
		...
		printf ("%d\n", (n->parentArr)->size);
		delSwapElementWithList (l, n->parentArr, n->children);
		printf ("%d\n", (n->parentArr)->size);
